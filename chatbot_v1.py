        ...
        try:
            if product_type == 'ssd':
                ...
                if all(product.values()):
                    ...
                    # Add to main database for search functionality
                    cursor = self.conn.cursor()
                    cursor.execute('''
                        INSERT INTO products 
                    ...
                    ''', (
                        ...
                        f"Interface: {product['interface']}, Model: {product['model']}",
                ...
                # Get values from Memory input fields
                product = {
                ...
                if all(product.values()):
                    # Generate auto-incrementing number
                    stt = len(self.memory_database) + 1
                    
                    # Add to database list
                    product['stt'] = stt
                    self.memory_database.append(product)
                    ...
                    self.clear_memory_fields()
                    self.show_message("Success", "Memory product added successfully!")
                else:
                    self.show_message("Error", "Please fill all fields!")
            
            elif product_type == 'motherboard':
                # Get values from Motherboard input fields
                product = {
                    'product': self.mb_product.get(),
                    'chipset': self.mb_chipset.get(),
                ...
                if all(product.values()):
                    # Generate auto-incrementing number
                    stt = len(self.motherboard_database) + 1
                    
                    # Add to database list
                    product['stt'] = stt
                    self.motherboard_database.append(product)
                    ...
                    ''', (
                        ...
                        price_num,
                        product['specifications'],
                        f"Chipset: {product['chipset']}, Model: {product['model']}",
                        product['stock_status'],
                        'Manual Entry',
                        datetime.now().isoformat()
                    ))
                    ...
                    self.mb_tree.insert('', 'end', values=(
                        stt, product['product'], product['chipset'], product['model'],
                        product['specifications'], product['stock_status'], product['price']
                    ))
                    
                    self.clear_motherboard_fields()
                    self.show_message("Success", "Motherboard product added successfully!")
                else:
                    self.show_message("Error", "Please fill all fields!")
...
class ExcelImportDialog:
    ...
    def add_sample_products(self):
        ...
        sample_products = [
            # SSD Products - from Excel template
            {
                'name': 'E130 256GB',
                'description': 'SSD (SOLID STATE DRIVE)',
                'category': 'SSD',
                'price': 600000,
                'features': 'Tốc độ 3200M/s Read 2700MB Write',
                'specifications': 'M2 NVMe',
            ...
            },
            {
                ...
                'category': 'SSD',
                'price': 900000,
                'features': 'Speed 3500M/s Read 3200MB Write',
                'specifications': 'M2 NVME',
                'availability': 'Còn hàng'
            },
            # Memory Products - from Excel template
            {
                'name': 'U3200I-C22 16GB',
                'description': 'Desktop Memory',
                'category': 'Memory',
                'price': 600000,
                'features': '3200Mhz 1.2V (Jedec), CAS Latency 22 chỉ tương thích với CPU Intel',
                'specifications': 'DDR4 UDIMM',
                'availability': 'còn hàng'
            },
            {
                'name': 'S3200I-C22 8GB',
                ...
                'category': 'Memory',
                'price': 400000,
                'features': '3200Mhz 1.2V (Jedec) CAS Latency 22 chỉ tương thích với CPU Intelv',
                'specifications': 'DDR4 SODIM',
                'availability': 'Còn hàng'
            },
            {
                'name': 'U5600-C46 16GB',
                ...
                'category': 'Memory',
                'price': 130000,
                'features': '5600Mh 1.1V (Jedec) CAS Latency 46 tương thích với CPU Intel và AMD',
                'specifications': 'DDR5 UDIMM',
                'availability': 'còn hàng'
            },
            # Motherboard Products - from Excel template
            {
                'name': 'H610M-HDV',
                'description': 'Intel Motherboard',
                'category': 'Motherboard',
                'price': 1650000,
                'features': 'Hỗ trợ CPU Intel thế hệ 12 13 14, sử dụng DDR4 cổng xuất hình: HDMI, Display Port, VGA. 2 USB 3.2, 2 USB 2.0',
                'specifications': 'H610',
                'availability': 'Còn hàng'
            },
            {
                ...
                'category': 'Motherboard',
                'price': 1890000,
                'features': 'Hỗ trợ CPU Intel thế hệ 12 13 14, sử dụng DDR4 cổng xuất hình: HDMI, Display Port, VGA. 4 USB 3.2, 2 USB 2.0, Có đèn led chẩn đoán lỗi.',
                'specifications': 'B760',
                'availability': 'Còn hàng'
            }
        ]
